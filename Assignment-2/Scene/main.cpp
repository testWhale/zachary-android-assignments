 ...
 *
 * Mouse interaction mirrors the touch pipeline used on Android:
 *   Left button press   → TouchEventDown    (tap-vs-swipe timer starts)
 *   Mouse move (held)   → TouchEventMove    (drag distance + velocity)
 *   Left button release → TouchEventRelease (tap toggles the fan; boost then coasts down)
 *
 * Keys 3 / 4 / 5 -> stretch goal: Renderer::SetBladeCount(3/4/5).
 *
 * Assignment 2 - 3D Fan (Scene/Fan.h / Scene/Fan.cpp).
 */
...
static void mouseButtonCB(GLFWwindow* win, int button, int action, int /*mods*/)
{
    ...
    if (action == GLFW_PRESS) {
        g_mouseDown = true;
        Renderer::Instance().TouchEventDown(static_cast<float>(x), static_cast<float>(y));
    } else if (action == GLFW_RELEASE) {
        g_mouseDown = false;
        Renderer::Instance().TouchEventRelease(static_cast<float>(x), static_cast<float>(y));
    }
}
...
{
    if (g_mouseDown) {
        Renderer::Instance().TouchEventMove(static_cast<float>(x), static_cast<float>(y));
    }
}
...
// Stretch goal: 3 / 4 / 5 keys pick the blade count at runtime.
static void keyCB(GLFWwindow* /*win*/, int key, int /*scancode*/, int action, int /*mods*/)
{
    if (action != GLFW_PRESS) return;
    switch (key) {
        case GLFW_KEY_3: Renderer::Instance().SetBladeCount(3); break;
        case GLFW_KEY_4: Renderer::Instance().SetBladeCount(4); break;
        case GLFW_KEY_5: Renderer::Instance().SetBladeCount(5); break;
        default: break;
    }
}
        ...
        switch (ev.type) {
            ...
            case SDL_MOUSEBUTTONDOWN:
                if (ev.button.button == SDL_BUTTON_LEFT) {
                    ...
                    Renderer::Instance().TouchEventDown(
                        static_cast<float>(ev.button.x), static_cast<float>(ev.button.y));
                }
                break;
            ...
            case SDL_KEYDOWN:
                ...
                switch (ev.key.keysym.sym) {
                    case SDLK_3: Renderer::Instance().SetBladeCount(3); break;
                    case SDLK_4: Renderer::Instance().SetBladeCount(4); break;
                    case SDLK_5: Renderer::Instance().SetBladeCount(5); break;
                    default: break;
                }
                break;
            default:
...
int main()
{
    ...
    std::cout << "Click: toggle fan | Click-drag: spin faster | Release: coasts down\n";
    std::cout << "Keys 3/4/5: blade count | ESC: quit\n";
